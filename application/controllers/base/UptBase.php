...
class ApplicationBase extends CI_Controller {
    ...
    protected function base_view_app() {
        ...
        // get user login
        $session = $this->tsession->userdata('session_upt');
        if (!empty($session)) {
            $this->com_user = $this->m_account->get_user_profil(array($session['user_id'], $session['role_id']));
            $this->user_instansi = $this->m_account->get_instansi_by_id($this->com_user['user_instansi']);
            // image
            ...
            $this->smarty->assign("user_instansi", $this->user_instansi);
        }
        // display global link
    ...
    protected function display($tmpl_name = 'base/upt/document.html') {
        // --
        $this->smarty->assign("template_sidebar", "base/upt/sidebar.html");
        // set template
    ...
    private function _display_site_title() {
        $this->portal_id = $this->config->item('portal_upt');
        // site data
    ...
    private function _display_current_page() {
        ...
        $url_menu = (empty($url_menu)) ? $this->config->item('default_upt') : $url_menu;
    ...
    protected function _set_page_rule($rule) {
        if (!isset($this->role_tp[$rule]) OR $this->role_tp[$rule] != "1") {
            ...
            redirect('login/uptforbidden/page/' . $this->nav_id);
        }
    }
