...
class m_ipk extends CI_Model {
    ...
    function insert($params) {
        $sql = "INSERT
                INTO TAD_IPK(FS_KD_PEG, FS_PENDIDIKAN, FS_JABATAN, FS_BAGIAN, FS_MASA_KERJA, FS_TUGAS_LAIN, FS_IKU, FS_IKI, FS_PENGURANG, 
                FS_PENGURANG_KET, PERIODE, TAHUN,FN_JML_IPK,FS_POTONGAN, mdb, mdd_date, mdd_time)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    
    function insert_pendapatan($params) {
        $sql = "INSERT
                INTO TAD_IPK2(PERIODE, TAHUN, FN_JML_PENDAPATAN, mdb, mdd_date, mdd_time)
                VALUES (?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }
    ...
    function update($params) {
        $sql = "UPDATE TAD_IPK 
            SET FS_PENDIDIKAN = ?, FS_JABATAN = ?, FS_BAGIAN = ?, FS_MASA_KERJA=?,FS_TUGAS_LAIN=?,
            FS_IKU = ?,FS_IKI=?, FS_PENGURANG=?,FS_PENGURANG_KET=?,PERIODE=?,TAHUN=?,FN_JML_IPK=?,
            FS_POTONGAN = ?, mdb=?,mdd_date=?,mdd_time=?
            WHERE FS_KD_TRS = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function delete($params) {
        $sql = "DELETE FROM arsip_kat_dokumen WHERE kategori_id = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_data_peg_by_search($params) {
        $sql = "SELECT FS_NM_PEG,a.FS_KD_PEG,FS_NM_BAGIAN,FS_KD_GOLONGAN,FS_REK_BANK,
                DATEDIFF(year, FD_TGL_DINAS, GETDATE()) AS MasaKerja,FS_NM_PENDIDIKAN,
    ...
    function get_ipk_by_search($params) {
        $sql = "SELECT a.*,b.FS_NM_PEG,FS_NM_BAGIAN 
                ...
                WHERE c.FS_KD_BAGIAN LIKE ? AND PERIODE = ? AND TAHUN = ?
