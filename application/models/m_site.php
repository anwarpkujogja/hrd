...
class m_site extends CI_Model {
    ...
    function get_current_page($params) {
        $sql = "SELECT TOP 1 * FROM PKU.dbo.tad_com_menu WHERE nav_url = ? ORDER BY nav_no DESC ";
    ...
    function get_navigation_user_by_parent($params) {
        $sql = "SELECT * FROM PKU.dbo.tad_com_menu a
                INNER JOIN PKU.dbo.tad_com_role_menu b ON a.nav_id = b.nav_id
                INNER JOIN PKU.dbo.tad_com_role_user c ON b.role_id = c.role_id
    ...
    function get_parent_group_by_idnav($int_parent, $limit) {
        $sql = "SELECT a.nav_id, a.parent_id FROM PKU.dbo.tad_com_menu a WHERE a.nav_id = ?
    ...
    function get_user_authority($user_id, $id_group) {
        $sql = "SELECT a.user_id FROM PKU.dbo.tad_com_user a
    ...
    function get_user_authority_by_nav($params) {
        $sql = "SELECT b.* FROM PKU.dbo.tad_com_menu a
                INNER JOIN PKU.dbo.tad_com_role_menu b ON a.nav_id = b.nav_id
                INNER JOIN PKU.dbo.tad_com_role c ON b.role_id = c.role_id
                INNER JOIN PKU.dbo.tad_com_role_user d ON c.role_id = d.role_id
